...
**تعريف:**  
نموذج **OSI** هو **اتفاق عالمي** يحدد بروتوكولًا موحدًا لتواصل الأجهزة في الشبكات،  
فبدونه لا يستطيع أي جهاز فهم الآخر.
...
|---|---|---|---|---|
|1️⃣|**Physical**|الفيزيائية|إرسال **البتات (Bits)** كإشارات كهربائية أو ضوئية عبر الوسيط|كابلات UTP/Fiber، موصلات RJ45، أجهزة Hubs وRepeaters|
|2️⃣|**Data Link**|رابط البيانات|عنونة الأجهزة (MAC)، كشف الأخطاء (CRC)، تجميع البتات في إطارات (Frames)|Switches|
|3️⃣|**Network**|الشبكة|تحديد العناوين (IP)، اختيار أفضل مسار (Routing)|Routers، بروتوكولات OSPF وBGP|
|4️⃣|**Transport**|النقل|تجزئة البيانات، التحكم في التدفق، تصحيح الأخطاء، اختيار البروتوكول (TCP/UDP)|TCP، UDP|
|5️⃣|**Session**|الجلسة|إنشاء وإدارة الجلسات بين الأجهزة، المصادقة والتفويض|Authentication، Session Management|
|6️⃣|**Presentation**|العرض|التشفير، الضغط، تحويل البيانات إلى صيغة مفهومة|SSL/TLS، ASCII|
|7️⃣|**Application**|التطبيق|واجهة المستخدم للتفاعل مع الشبكة|HTTP، SMTP، DNS، FTP|

---

#### 🧩 **ملاحظات سريعة – طبقات نموذج OSI**

|**الطبقة (Layer)**|**الوظيفة الأساسية**|**أمثلة / ملاحظات**|
|---|---|---|
|**Layer 7 – Application**|يتعامل مباشرة مع المستخدم والتطبيقات.|مثل: HTTP, SMTP, DNS|
|**Layer 6 – Presentation**|يجعل البيانات قابلة للفهم بين الأنظمة المختلفة.|مثل: SSL/TLS, JPEG, ASCII|
|**Layer 5 – Session**|ينسّق الاتصالات ويعيدها عند الانقطاع.|إدارة الجلسات (Sessions) بين الطرفين|
|**Layer 4 – Transport**|يضمن تسليم البيانات بدقة (**TCP**) أو سرعة (**UDP**).|تقسيم البيانات – Flow Control|
|**Layer 3 – Network**|يختار المسارات باستخدام العناوين المنطقية.|بروتوكولات: IP, ICMP, OSPF|
|**Layer 2 – Data Link**|ينقل الإطارات ويكشف الأخطاء.|MAC Address, Ethernet, Switches|
|**Layer 1 – Physical**|يُعنى بالناقل المادي والإشارات.|كابلات، إشارات كهربائية أو ضوئية|
...
|**الطبقة (Layer)**|**المهمة**|**المثال / التوضيح**|
|---|---|---|
|**Layer 7 – Application**|تعامل مباشر مع المستخدم والتطبيقات.|`https://mail.google.com` – تطبيق فعلي يتفاعل مع المستخدم.|
|**Layer 6 – Presentation**|تشفير وتحويل تنسيقات البيانات.|**SSL encryption** – تشفير البيانات بين الطرفين.|
|**Layer 5 – Session**|إدارة الجلسة بين الأنظمة.|يربط الـ **Presentation** مع الـ **Transport** للحفاظ على الجلسة.|
|**Layer 4 – Transport**|تقسيم البيانات وضمان تسليمها.|**TCP encapsulation** – تغليف البيانات والتحكم بالتدفق.|
|**Layer 3 – Network**|إضافة العنوان المنطقي وتحديد المسار.|**IP encapsulation** – إضافة عنوان IP للوجهة.|
|**Layer 2 – Data Link**|تغليف الإطار وإضافة العناوين الفيزيائية.|**Ethernet Frame** – إضافة عنوان MAC.|
|**Layer 1 – Physical**|إرسال الإشارات عبر الوسيط المادي.|**Electrical / Optical Signals** – نقل البتات فعليًا.|
...
---
## 🔹 **Networking Appliances – الأجهزة الفيزيائية والافتراضية**

|**الجهاز**|**الوظيفة الأساسية**|**ملاحظات / الطبقة**|
|---|---|---|
|**Router (الراوتر)**|يربط الشبكات المختلفة ويحدد أفضل مسار لنقل البيانات.|يعمل في **Layer 3 – Network**|
|**Switch (السويتش)**|يربط الأجهزة داخل نفس الشبكة المحلية (LAN).|يعمل في **Layer 2 – Data Link**، ويدعم **Layer 3** في السويتشات المُدارة|
|**Firewall (الجدار الناري)**|يفلتر الترافيك ويفرض سياسات الأمان بين الشبكات.|يمكن أن يكون **Hardware** أو **Software**|
|**IDS / IPS**|**IDS:** يكتشف الهجمات.  <br>**IPS:** يكتشف ويمنعها.|تعمل غالبًا مع **Firewall** و**SIEM**|
|**Load Balancer**|يوزع الحمل (Traffic) على عدة خوادم لزيادة الكفاءة والتوافرية.|يُستخدم في بيئات السيرفرات والتطبيقات|
|**Proxy Server**|يعمل كوسيط بين المستخدم والإنترنت لتصفية أو تخزين البيانات (Caching / Filtering).|غالبًا في **Layer 7 – Application**|
|**NAS (Network-Attached Storage)**|تخزين مشترك على الشبكة عبر بروتوكولات مثل SMB أو NFS.|يُستخدم في الشبكات الصغيرة والمتوسطة|
|**SAN (Storage Area Network)**|شبكة تخزين عالية السرعة تستخدم بروتوكولات مثل **Fibre Channel**.|مناسبة للمراكز الكبيرة (Data Centers)|
|**Wireless Access Point (AP)**|يربط الأجهزة اللاسلكية بالشبكة السلكية.|يعمل في **Layer 2**|
|**Wireless Controller**|يدير عدة نقاط وصول (Access Points) مركزياً.|يستخدم في الشبكات اللاسلكية المؤسسية|
