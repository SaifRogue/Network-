...
هو عملية **تحديد المسارات يدويًا** داخل جهاز التوجيه (**Router**) من قبل مسؤول الشبكة.  
في هذا النوع من التوجيه، لا يقوم الراوتر باكتشاف المسارات تلقائيًا مثل بروتوكولات التوجيه الديناميكي (**OSPF**, **EIGRP**)، بل يعتمد على **إدخال يدوي مباشر** لتحديد الشبكة الوجهة (**Destination Network**) والمسار (**Next-Hop**).
...
|الخاصية|الوصف|
|---|---|
|**الإدارة اليدوية**|يتم إدخال كل مسار يدويًا من قبل مسؤول الشبكة.|
|**لا يستهلك موارد المعالج**|لأنه لا يتبادل تحديثات مع راوترات أخرى.|
|**موثوق ومستقر**|لا يتغير إلا إذا تم تعديله يدويًا.|
|**قابلية التوسع محدودة**|غير مناسب للشبكات الكبيرة كثيرة التغيّر.|

---

### ✅ **المميزات**

- تحكم كامل بمسارات التوجيه.
    
- لا يولّد ترافيك تحديثات مثل البروتوكولات الديناميكية.
    
- أداة ممتازة للاختبار أو للشبكات الصغيرة الثابتة.
...
- يحتاج تعديل يدوي عند أي تغيير في الشبكة.
    
- غير فعّال في البيئات المتعددة المسارات أو الديناميكية.
    
- يمكن أن يسبب **انقطاعًا** في الاتصال إذا لم يتم تحديث المسار يدويًا عند حدوث تغيّر.
...
### 📶 **Network Identifiers (معرّفات الشبكة)**

|المصطلح|الاسم الكامل|الوصف|
|---|---|---|
|**SSID**|_Service Set Identifier_|اسم الشبكة اللاسلكية الذي يظهر للمستخدمين (مثل: `Tuwaiq_WiFi`).|
|**BSSID**|_Basic Service Set Identifier_|عنوان **MAC الفريد** لكل نقطة وصول (Access Point). يحدد الـ AP المحدد داخل الشبكة.|
|**ESSID**|_Extended Service Set Identifier_|مجموعة من نقاط الوصول **تستخدم نفس SSID** لتشكيل شبكة موحدة، مما يسمح للمستخدم **بالتجوال (Roaming)** بينها بسلاسة.|

---

📘 **مثال تطبيقي:**

> 🏢 مبنى يحتوي على 3 نقاط وصول كلها تحمل SSID باسم  
> **"Tuwaiq_WiFi"**
> 
> - كل واحدة لها **BSSID مختلف (MAC Address)**
>     
> - معًا تشكّل **ESSID واحد** يسمح بالتجوال السلس داخل المبنى.
>
...
|البروتوكول / النمط|الاسم الكامل|الوصف|الاستخدام الشائع|
|---|---|---|---|
|**WPA2 (AES/CCMP)**|_Wi-Fi Protected Access 2_|بروتوكول آمن وشائع يعتمد على **تشفير AES** مع خوارزمية **CCMP**.|أغلب الشبكات الحديثة.|
|**WPA3 (SAE)**|_Wi-Fi Protected Access 3_ / _Simultaneous Authentication of Equals_|أحدث من WPA2، يوفّر حماية أقوى ضد **هجمات Brute Force**.|بيئات تحتاج أمان أعلى.|
|**PSK**|_Pre-Shared Key_|كلمة مرور موحدة لكل المستخدمين (مثل Wi-Fi المنزلي).|المنازل والمكاتب الصغيرة.|
|**Enterprise (802.1X + RADIUS)**|_Enterprise Authentication_|كل مستخدم لديه **بيانات اعتماد خاصة** (اسم مستخدم وكلمة مرور). يعتمد على **خادم RADIUS**.|الشركات والمؤسسات الكبرى.|

---

📘 **ملاحظات مهمة:**

- **AES** = Advanced Encryption Standard → معيار التشفير القوي.
    
- **CCMP** = Counter Mode CBC-MAC Protocol → يضيف مصادقة وسلامة إضافية.
    
- **RADIUS** = Remote Authentication Dial-In User Service → خادم مصادقة مركزي.
