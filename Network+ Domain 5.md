...
هذه الأدوات تُستخدم لتحليل، مراقبة، واكتشاف الأعطال في الشبكات.  
تنقسم إلى ثلاث فئات رئيسية:

1. **أدوات تحليل البروتوكولات (Protocol Analyzers)**
    
2. **أوامر سطر الأوامر (Command-Line Tools)**
    
3. **أدوات متقدمة لاكتشاف الأجهزة وتحليل الأداء**
    

---

### 🔹 **1️⃣ Protocol Analyzer**

|الأداة|الوصف|الاستخدام|
|---|---|---|
|**Wireshark**|أداة لتحليل الترافيك على مستوى الحزم (Packets).|تستخدم لاكتشاف الأخطاء أو الهجمات أو اختبار الاتصالات بين الأجهزة.|
|**tcpdump**|أداة CLI لالتقاط الحزم (Packet Capture) مباشرة من الواجهة.|مفيدة في السيرفرات أو بيئات Linux.|

📌 **تلميح:**  
Wireshark → واجهة رسومية GUI  
tcpdump → سطر أوامر CLI

---

### 🔹 **2️⃣ Command-Line Tools (أوامر سطر الأوامر)**

|الأداة|الوظيفة|الوصف السريع|
|---|---|---|
|**ping**|اختبار الاتصال|يرسل **ICMP Echo Request** للتأكد من وجود الجهاز واستجابته.|
|**traceroute / tracert**|تتبع المسار|يحدد المسار بين المصدر والوجهة ويظهر نقاط التوقف (hops).|
|**nslookup / dig**|تحليل DNS|nslookup: بسيط وسريع.  <br>dig: أكثر تفصيلًا ويُستخدم في Linux.|
|**netstat**|عرض الاتصالات|يعرض المنافذ المفتوحة، الجلسات النشطة، وعلاقات TCP/UDP.|
|**ip / ifconfig / ipconfig**|إعدادات الشبكة|عرض وضبط إعدادات الشبكة (IP, Subnet, Gateway).|
|**arp**|إدارة جدول ARP|يعرض أو يحرر جدول الربط بين IP و MAC.|

📘 **أمثلة عملية:**

- `ping 8.8.8.8` → اختبار الإنترنت
    
- `tracert google.com` → تتبع المسار
    
- `ipconfig /all` → عرض إعدادات الـ NIC
    
- `arp -a` → عرض الأجهزة المتصلة محليًا
...
|الأداة|الوصف|الاستخدام|
|---|---|---|
|**Nmap (Network Mapper)**|أداة لاكتشاف الأجهزة المفتوحة على الشبكة وتحديد المنافذ (Ports) والخدمات.|فحص الثغرات أو اكتشاف الأجهزة النشطة.|
|**LLDP / CDP**|بروتوكولات اكتشاف الجيران (Link Layer Discovery Protocol / Cisco Discovery Protocol).|تُستخدم لمعرفة الأجهزة المجاورة للسويتش (اسم الجهاز، IP، المنفذ).|
|**Speed Tester**|أداة لقياس سرعة الاتصال (Download/Upload).|تساعد في تحديد بطء الإنترنت أو مشاكل مزود الخدمة.|
...
|---|---|---|
|**تحليل البروتوكولات**|Wireshark, tcpdump|مراقبة الترافيك وفهم الحزم.|
|**سطر الأوامر**|ping, tracert, nslookup, netstat, ipconfig, arp|تشخيص الاتصال والمشاكل الأساسية.|
|**تحليل متقدم**|Nmap, LLDP, CDP, Speed Test|اكتشاف الأجهزة وقياس الأداء.|
...
## ⚙️ **Hardware Tools  5.5**

---

### 🧠 **الفكرة العامة**

هذه الأدوات تُستخدم **فعليًا (Physically)** لاكتشاف الأعطال في الكابلات والشبكات،  
بعكس أدوات البرمجيات التي تعمل من داخل النظام.

كل أداة من هذه الأدوات تساعد الفني في **تحديد، تتبع، أو اختبار الإشارة** في مكونات الشبكة.

---

### 🔹 **1️⃣ Toner Probe (جهاز التتبع)**

|العنصر|الشرح|
|---|---|
|**الوظيفة**|تحديد مسار كابل الشبكة داخل الجدران أو بين الرفوف.|
|**آلية العمل**|يتكوّن من جزئين:  <br>🔸 **Toner:** يرسل إشارة صوتية عبر الكابل.  <br>🔸 **Probe:** يلتقط الإشارة لتحديد موقع الكابل أو نهايته.|
|**الفائدة**|يستخدمه الفني لمعرفة وجهة الكابل بدون الحاجة إلى قطعه أو فصله.|
|**المثال العملي**|تتبع كابل غير مُعلَّم داخل غرفة السيرفرات لتحديد وجهته.|

---

### 🔹 **2️⃣ Cable Tester (جهاز اختبار الكابلات)**

| العنصر      | الشرح                                                                                                       |
| ----------- | ----------------------------------------------------------------------------------------------------------- |
| **الوظيفة** | اختبار توصيل الأسلاك داخل كابل Ethernet.                                                                    |
| **يكشف عن** | الأسلاك **المعكوسة (Reversed)** أو **المقطوعة (Open)** أو **القصيرة (Shorted)**.                            |
| **الأنواع** | 🔹 **Basic Cable Tester:** يختبر فقط التوصيل.  <br>🔹 **Cable Certifier:** يقدم قياسات جودة الإشارة والطول. |
| **الفائدة** | التأكد من سلامة الكابل قبل التركيب في الشبكة.                                                               |

---

### 🔹 **3️⃣ Network Tap (TAP – Test Access Point)**

|العنصر|الشرح|
|---|---|
|**الوظيفة**|جهاز يُركّب بين سويتشين أو بين جهاز وشبكة لنسخ الترافيك بدون التأثير عليه.|
|**الفائدة**|يُستخدم في **مراقبة الشبكات وتحليلها** مع أدوات مثل Wireshark.|
|**الميزة**|لا يسبب تأخير أو فقد للحزم (Unlike port mirroring).|
|**المثال العملي**|توصيل TAP بين الراوتر والفَيَروال لجمع الترافيك بشكل غير تدخلي.|
...
|العنصر|الشرح|
|---|---|
|**الوظيفة**|تحليل الشبكات اللاسلكية (Wi-Fi) من حيث القنوات، القوة، والتداخل.|
|**الفائدة**|تحديد **القنوات المزدحمة (Channel Overlap)** وتحسين التغطية اللاسلكية.|
|**الأشكال**|تطبيقات للهاتف، أو أدوات احترافية مثل Ekahau / NetSpot.|
|**المثال العملي**|اكتشاف أن قناتك في 2.4GHz تتداخل مع قنوات الجيران.|

---

### 🔹 **5️⃣ Visual Fault Locator (VFL)**

|العنصر|الشرح|
|---|---|
|**الوظيفة**|جهاز يُصدر ضوء ليزر أحمر عبر **كابل الألياف الضوئية** لتحديد مكان الانقطاع.|
|**الفائدة**|يمكنك رؤية الضوء يتوقف عند مكان الكسر أو الانحناء في الليف.|
|**الاستخدام**|اختبار وصيانة الألياف الضوئية (Fiber).|
|**المثال العملي**|في كابل SMF طويل، يضيء الضوء حتى النقطة التي فيها قطع أو خلل.|

---

### ✅ **ملخص سريع**

|الأداة|الاستخدام الأساسي|الفائدة|
|---|---|---|
|**Toner Probe**|تتبع الكابلات|تحديد موقع الكابل داخل الجدار أو الرف|
|**Cable Tester**|اختبار التوصيل|اكتشاف الأسلاك المقطوعة أو المعكوسة|
|**Network TAP**|مراقبة الشبكة|نسخ الترافيك للتحليل دون التأثير عليه|
|**Wi-Fi Analyzer**|تحليل الإشارة اللاسلكية|كشف التداخل وتحسين القنوات|
|**VFL**|فحص الألياف الضوئية|تحديد مكان الكسر في الليف بصريًا|


---
---

## 🖥️ **Basic Network Device Commands  5.5**

---

### 💪 **هانت يا بطل! وصلت لآخر جزء في المنهج 🎯**

باقي الخطوات الأخيرة قبل الإنهاء، وهنا تبدأ الأوامر الحقيقية اللي يستخدمها كل فني شبكات داخل **الـ Switch والـ Router**.  
الأوامر الجاية تعطيك **عين داخل الجهاز نفسه**: تعرف من متصل، كيف التوجيه شغال، ومن وين يدخل الترافيك ويطلع.

---

### 🧠 **الفكرة العامة**

هذه الأوامر تُستخدم على أجهزة الشبكة لمعرفة **الحالة العامة وتشخيص الأعطال**.  
كل أمر يعطيك رؤية مختلفة، من **الجداول (Tables)** إلى **المنافذ (Interfaces)** إلى **الطاقة (PoE)**.

---

### 🔹 **1️⃣ show mac-address-table**

|العنصر|الشرح|
|---|---|
|**الوظيفة**|عرض جدول عناوين MAC المخزّنة في السويتش.|
|**الفائدة**|معرفة الأجهزة المتصلة بكل منفذ من منافذ السويتش.|
|**الاستخدام**|تشخيص مشاكل الربط أو التأكد من تسجيل الأجهزة بشكل صحيح.|
|**مثال عملي**|`Switch# show mac-address-table`|

📘 **معلومة مهمة:**  
الـ **MAC Table** تربط بين **عنوان MAC** و **رقم المنفذ (Port)** الذي يتصل به.
...
|العنصر|الشرح|
|---|---|
|**الوظيفة**|عرض جدول التوجيه (Routing Table).|
|**الفائدة**|يوضح كيف يختار الراوتر المسار المناسب لكل شبكة وجهة.|
|**الاستخدام**|تأكيد وجود المسارات الصحيحة وحالة التوجيه الديناميكي أو الثابت.|
|**مثال عملي**|`Router# show ip route`|

📘 **تذكّر:**  
كل **Route** يحتوي على **Network Destination + Next Hop + Interface**.
...
|العنصر|الشرح|
|---|---|
|**الوظيفة**|عرض حالة المنافذ (Interfaces) وسرعتها وإحصاءاتها.|
|**الفائدة**|كشف الأعطال مثل **duplex mismatch** أو **packet drops**.|
|**الاستخدام**|فحص حالة المنفذ عند انقطاع الاتصال أو ضعف السرعة.|
|**مثال عملي**|`Switch# show interfaces status`|
...
|العنصر|الشرح|
|---|---|
|**الوظيفة**|عرض الإعدادات الحالية في الجهاز (Configuration).|
|**الفائدة**|مراجعة الإعدادات النشطة بعد التعديل أو قبل الحفظ.|
|**الاستخدام**|مقارنة الإعداد الحالي مع النسخة المحفوظة.|
|**مثال عملي**|`Router# show running-config` أو `Router# show startup-config`|
...
|العنصر|الشرح|
|---|---|
|**الوظيفة**|عرض جدول ربط **IP ↔ MAC** (ARP Table).|
|**الفائدة**|تشخيص مشاكل الاتصال داخل الشبكة المحلية (LAN).|
|**الاستخدام**|التأكد من وجود الـ MAC الصحيح للمضيف المطلوب.|
|**مثال عملي**|`Switch# show arp` أو `Router# show ip arp`|
...
|العنصر|الشرح|
|---|---|
|**الوظيفة**|عرض قائمة الـ VLANs على السويتش والمنفذ المرتبط بكل VLAN.|
|**الفائدة**|التأكد من توزيع VLANs الصحيحة بين المنافذ.|
|**الاستخدام**|تشخيص مشاكل **VLAN Trunking** أو فصل الشبكات.|
|**مثال عملي**|`Switch# show vlan brief`|
...
|العنصر|الشرح|
|---|---|
|**الوظيفة**|عرض استهلاك الطاقة عبر منافذ **PoE** في السويتش.|
|**الفائدة**|معرفة الأجهزة التي تستقبل طاقة وعدد الواط المستخدم لكل منفذ.|
|**الاستخدام**|مفيد عند حدوث مشكلة **Power budget exceeded**.|
|**مثال عملي**|`Switch# show power inline`|
...
| الأمر                      | الوظيفة                      | الغرض                           |
| -------------------------- | ---------------------------- | ------------------------------- |
| **show mac-address-table** | عرض الأجهزة المتصلة بالمنافذ | لتشخيص مشاكل الاتصال            |
| **show route**             | عرض جدول التوجيه             | لفهم مسارات الشبكة              |
| **show interface**         | عرض حالة المنافذ             | لاكتشاف مشاكل السرعة أو الإشارة |
| **show config**            | عرض الإعدادات الحالية        | للمراجعة قبل الحفظ              |
| **show arp**               | عرض IP ↔ MAC                 | لتشخيص الاتصال داخل LAN         |
| **show vlan**              | عرض الشبكات الافتراضية       | لتصحيح إعدادات VLAN             |
| **show power**             | عرض استهلاك الطاقة           | لحل مشاكل PoE                   |



---
---
الله يكتب لكم التوفيق ❤️